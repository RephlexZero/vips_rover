...
def generate_launch_description():
    ...
    hardware_launch = IncludeLaunchDescription(
        ...
        launch_arguments={
            'urdf_model': LaunchConfiguration('urdf_model', default=os.path.join(pkg_rover_description, 'urdf/rover.urdf.xacro')),
            'vips_serial_port': LaunchConfiguration('vips_serial_port', default='/dev/ttyUSB0'),
            'can_interface': LaunchConfiguration('can_interface', default='can0'),
        }.items()
    )
    
    return LaunchDescription([
        DeclareLaunchArgument('urdf_model', default_value=os.path.join(pkg_rover_description, 'urdf/rover.urdf.xacro')),
